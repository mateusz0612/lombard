...
export const LoanInfo: FC<ILoanInfo> = ({ loanInfo, loanItemName }) => {
  ...
  useEffect(() => {
    if (!isLoadingClients) {
      setCurrentClient(
        clients.filter(
          (client) => client["personalIdNumber"] === loanInfo.personalIdNumber
        )[0]
      );
    }
  ...
  return (
    <LoanInfoWall>
      ...
      <Styled.Name>
        <LoanInfoBrick
          value={`${currentClient.firstName} ${currentClient.secondName}`}
      ...
      <Styled.LoanDate>
        <LoanInfoBrick
          value={format(loanInfo.dateOfLoan.toDate(), "MM/dd/yyyy")}
          label="Data wzięcia pożyczki"
        />
      </Styled.LoanDate>
      <Styled.Interest>
        <LoanInfoBrick
          value={loanInfo.interest}
          label="Stopa procentowa w skali miesiąca"
        />
      </Styled.Interest>
      <Styled.ReturnPrice>
      ...
      <Styled.LoanItemName>
        <LoanInfoBrick value={loanItemName} label="Zastawiony przedmiot" />
      </Styled.LoanItemName>
    </LoanInfoWall>
  );
};
